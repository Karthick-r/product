...
<a href="{{ url('zone') }}" class="m-nav__link">
            ...
            <div class="m-portlet m-portlet--full-height m-portlet--tabs  ">
                <div class="tab-content">
                    <div class="tab-pane active" id="m_user_profile_tab_1">
                        <form class="m-form m-form--fit m-form--label-align-right" action="{{ route ('zone.update', $zone->id ) }}" method="POST">
                            ...
                            <div class="m-portlet__body">
                                ...
                                <div class="form-group m-form__group row">
                                        <div class="col-10 ml-auto">
                                            <h3 class="m-form__section">Edit Zone</h3>
                                        </div>
                                    </div>
                                ...
                                <div class="form-group m-form__group row   @if ($errors->has('state')) {{'has-danger'}} @endif">
                                        <label for="example-text-input" class="col-2 col-form-label text-left"> Select state</label>
                                        <div class="col-4">
                                                <select class="form-control m-input" name="state" id="exampleSelect1">
                                                       
                                                    @if(count($states)>=1);
                                                    @foreach($states as $state)
                                                <option value="{{$state->id}}" @if($state->id== $zone->state_id) {{'selected'}}@endif>{{$state->name}}</option>
                                                    @endforeach
                                                    @else
                                                    <option value="0" >Create state First</option>
                                                    @endif        
                            ...
                            <div class="form-group m-form__group row   @if ($errors->has('name')) {{'has-danger'}} @endif">
                                    <label for="example-text-input" class="col-2 col-form-label text-left"> zone Name</label>
                                    <div class="col-4">
                                        <input class="form-control m-input" name="name" type="text" value="{{ $zone->name }}" >
                                        @if ($errors->has('name'))
